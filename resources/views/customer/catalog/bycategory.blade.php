...
<div class="container mx-auto px-4 py-6">
    <h1 class="text-2xl font-bold text-green-800 mb-4">Tanaman Kategori: {{ $category->name }}</h1>
        ...
        @forelse ($plants as $plant)
            <div class="bg-white rounded-lg shadow p-3 flex flex-col items-center text-center">
                ...
                <form action="{{ route('customer.catalog.buy', $plant->id) }}" method="POST" class="mt-2 w-full">
                    @csrf
                    <button type="submit"
                            class="w-full bg-blue-600 hover:bg-blue-700 text-white text-xs px-2 py-1 rounded">
                        Beli 🌱
                    </button>
                </form>
            </div>
        @empty
            <p class="text-gray-500 col-span-full">Tidak ada tanaman di kategori ini.</p>
        @endforelse
    </div>
</div>
@endsection
